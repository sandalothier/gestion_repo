package com.brain.gestion.web.rest;
...
import com.brain.gestion.config.KafkaProperties;
...
class GestionKafkaResourceIT {
    ...
    void setup() {
        ...
        GestionKafkaResource kafkaResource = new GestionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/gestion-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gestion-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
